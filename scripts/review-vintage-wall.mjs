// Read-only: screenshot the vintage "wall of photos" home + events across
// viewports and moods, plus lightbox keyboard/focus checks.
// Run (dev server must be up): node scripts/review-vintage-wall.mjs
import { chromium } from 'playwright';
import path from 'node:path';
import { VIEWPORTS, setMood, reviewDir } from './_review-common.mjs';

const OUT = reviewDir('vintage-wall');
  ...
  for (const vp of VIEWPORTS) {
    ...
    for (const mood of MOODS) {
      ...
      await page.screenshot({
        path: path.join(OUT, 'lightbox-open-' + vp.name + '-' + mood + '.png'),
        fullPage: false,
      });
      await page.keyboard.press('ArrowRight');
      await page.waitForTimeout(300);
      await page.screenshot({
        path: path.join(OUT, 'lightbox-next-' + vp.name + '-' + mood + '.png'),
        fullPage: false,
      });
      ...
      findings.push(
        '[esc-closes ' + vp.name + ' ' + mood + '] ' + (stillOpen === 0 ? 'OK closed' : 'FAIL still open'),
      );
    ...
    while (tabs < 40) {
      ...
      const onWall = await page.evaluate(
        () =>
          ...
          document.activeElement.classList.contains('wl-wall-item'),
      );
      if (onWall) {
        landed = true;
        break;
